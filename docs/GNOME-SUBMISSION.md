...
4. **Desktop acceptance.** Clock controls, backgrounds, group independence, lock/unlock, suspend/resume, Overview, and full-screen behavior have passed on the owner's physical GNOME desktop. Complete available multi-monitor, scaling, time-status persistence, and longer-running resource checks. These are release-quality checks, not a claim that EGO tests every behavior. Inspect logs for extension errors.
